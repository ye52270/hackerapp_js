...
function getData(method, url, option = false) {
    console.log(url);
...
function newsFeed() {
  ...
  `;
    ...
    template = template.replace("{{__next_page__}}", String(store.currentPage < totalPage ? store.currentPage + 1 : store.currentPage));
    updateView(template);
}
// -----------------------------------------  News Detail
function newsDetail() {
...
function makeComment(comments) {
  ...
  `);
        if (comments[i].comments.length > 0) commentString.push(makeComment(comments[i].comments));
    }
